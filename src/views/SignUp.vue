...
const defaultImage =
  'https://www.polytechnique.edu/sites/default/files/styles/annuaire/public/default_images/user.png?itok=y3Uo_Z7l'
...
const validateForm = (data) => {
  for (let entry of Object.entries(data)) {
    ...
    switch (entry[0]) {
      case 'name': {
        if (!validatorMap.name.validator(entry[1]))
          store.state.feedback = { status: 400, msg: validatorMap.name.error }
        break
      }
      case 'email': {
        if (!validatorMap.email.validator(entry[1]))
          store.state.feedback = { status: 400, msg: validatorMap.email.error }
        break
      }
      case 'password': {
        if (!validatorMap.password.validator(entry[1]))
          store.state.feedback = { status: 400, msg: validatorMap.password.error }
        break
      }
    }
  }
...
const handleSubmit = (e) => {
  ...
  if (store.state.feedback.status === null) {
    if (file) {
      store
        .saveUser(data, file)
        .then(() => (store.state.feedback = { status: 200, msg: 'Account created' }))
        .then(()=> setTimeout(() => router.push('/'), 300))
    } else {
      store.state.feedback = {
        status: 400,
        msg: 'Error in Image'
...
<template>
  <div class="wrapper">
    ...
    <div class="rest_container">
      ...
      <p></p>
      <span>
        © 2024 Interlink. Crafted With <i class="fa-solid fa-heart"> </i>&nbsp; And Coffee</span
      >
    </div>
  </div>
</template>
